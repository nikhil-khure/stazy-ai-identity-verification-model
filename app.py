...
PAN_LOGO_PATH = os.path.join(os.path.dirname(__file__), "assets", "pan_emblem_ref.jpg")
...
def student_verification():
    try:
        ...
        # ---------- BARCODE ----------
        barcode_value = decode_barcode(id_img)
        # barcode_value is passed so extract_student_code can trim extra digits
        # that Tesseract hallucinates from barcode stripes (OCR post-processing only).
        student_code_from_ocr = extract_student_code(ocr_text, barcode_value)
