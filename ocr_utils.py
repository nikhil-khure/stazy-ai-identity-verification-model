...
def extract_student_code(ocr_text, barcode_value=None):
    """
    Extract the numeric student ID code from OCR text of a College ID card.
    ...
    barcode_value (optional):
           When the decoded barcode value is supplied, P1/P2/P3 candidates are
           post-processed through _trim_to_barcode() before being returned.
           This removes the extra leading/trailing digits that Tesseract hallucinates
           from barcode stripes or card borders, producing an exact-length match
           without touching any decoding or matching logic.
    ...
    # Derive the clean digit-only barcode reference once (strip S/ST/SC prefix).
    # Used only for post-processing OCR noise; never changes the match logic.
    barcode_digits = None
    if barcode_value:
        barcode_digits = re.sub(r'^(SC|ST|S)', '', barcode_value.upper().strip())
        barcode_digits = re.sub(r'[^0-9]', '', barcode_digits)
        if not barcode_digits:
            barcode_digits = None

    def _trim_to_barcode(ocr_num):
        """
        Remove extra leading/trailing digits introduced by OCR noise.

        Tesseract commonly hallucinates a digit at the start or end of the
        barcode-preceded number because:
          - The first/last barcode bar is read as a '1' or '0'.
          - A card border pixel bleeds into the number.

        Strategy:
          1. If ocr_num already equals barcode_digits → return as-is.
          2. If barcode_digits is a substring of ocr_num → return barcode_digits
             (exact trim: the OCR string contains the truth plus noise digits).
          3. If lengths differ by exactly 1 → try stripping one char from the
             start, then the end, and return the one that matches.
          4. Otherwise → return ocr_num unchanged (let barcode_matches_ocr decide).

        This function ONLY performs post-processing trimming. It does not alter
        any decoding or matching logic.
        """
        if barcode_digits is None:
            return ocr_num
        if ocr_num == barcode_digits:
            return ocr_num
        # Exact substring: truth is cleanly contained inside the noisy OCR string
        if barcode_digits in ocr_num:
            return barcode_digits
        # Off by one digit at the leading or trailing edge
        if len(ocr_num) == len(barcode_digits) + 1:
            if ocr_num[1:] == barcode_digits:   # extra digit at start
                return barcode_digits
            if ocr_num[:-1] == barcode_digits:  # extra digit at end
                return barcode_digits
        return ocr_num
    ...
    m = re.search(r'\bS[TC]?\s*(\d{5,9})\b', cleaned)
    if m:
        return _trim_to_barcode(m.group(1))
    ...
    # P3 – standalone 6-9 digit number
    # Collect all candidates; if a barcode reference is available, pick the
    # candidate whose trimmed form best matches it.  Without a reference, fall
    # back to the last match (barcode-zone text is appended last by
    # _merge_ocr_results, so the student code under the barcode appears later
    # in the merged string than incidental numbers from the top of the card).
    nums = re.findall(r'\b(\d{6,9})\b', cleaned)
    if not nums:
        return None
    if barcode_digits is None:
        return nums[-1]
    # Pick the candidate that produces an exact match after trimming
    for candidate in nums:
        trimmed = _trim_to_barcode(candidate)
        if trimmed == barcode_digits:
            return trimmed
    # No candidate matched exactly — trim the last one and return it
    return _trim_to_barcode(nums[-1])
